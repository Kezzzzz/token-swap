  ...
  const filteredTokens = availableTokens
    .filter(
      (token) =>
        ...
        CHAIN_NAMES[token.chainId]?.toLowerCase().includes(searchQuery.toLowerCase())
    )
    .sort((a, b) => {
      ...
      <div ref={dropdownRef} className="relative">
        ...
        <div className="flex items-center justify-between gap-3">
          {selectedToken ? (
            ...
            <span className="flex-1 text-sm text-gray-500">Select token...</span>
          )}

          {/* Sparkline & Price Change (when token selected) */}
          {selectedToken && selectedSparkline && selectedSparkline.prices && selectedSparkline.prices.length > 1 && (
            <div className="flex flex-shrink-0 items-center gap-2">
              ...
              <div
                className={`text-xs font-semibold min-w-[45px] text-right ${
                  selectedSparkline.isPositive ? "text-green-500" : "text-red-500"
                }`}
              >
        ...
        {isOpen && (
          <div 
            className={`absolute left-0 right-0 z-50 overflow-hidden rounded-xl border border-gray-700/50 bg-[#1a1a20] shadow-2xl shadow-black/50 ${
